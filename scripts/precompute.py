...
def compute_step(model, batch, device):
    if model.ucg_rate > 0:
        batch["text"] = ["" if np.random.rand() < model.ucg_rate else t for t in batch["text"]]
    batch["text"] = model.tokenizer(
        batch["text"],
    ...
    encoder_hidden_states = model.text_encoder(batch["text"], return_dict=False)[0]

    latents = model.vae.encode(batch["image"].to(device)).latent_dist.sample() * model.vae.config.scaling_factor
    ...
    with wds.ShardWriter(str(save_path_pattern), maxsize=1e9, maxcount=100000) as sink:
        for _ in range(cli.config.repeat):
            for batch in tqdm(dataloader):
                ...
                model.to(device)
                result = compute_step(model, batch, device)
